...
const getAssociationsByDisciplineId = (disciplineId: number): DisciplineCourse[] => {
    return disciplineCourse.filter(( dc ) => dc.disciplineId === disciplineId );
}

const createAssociation = (disciplineId: number, courseId: number): DisciplineCourse => {
...
const deleteAssociation = (associationId: number) => {
    const index = disciplineCourse.findIndex( (dc) => { dc.id === associationId } );
